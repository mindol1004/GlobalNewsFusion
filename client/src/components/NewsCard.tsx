...
export default function NewsCard({
  ...
  const { translateNewsArticle, isTranslating, userLanguage } =
    useTranslationContext();
  const { toast } = useToast();

  // 번역된 기사 상태 관리
  const [translatedArticle, setTranslatedArticle] =
    useState<NewsArticle>(article);
    ...
    <div className="news-card bg-white dark:bg-neutral-800 rounded-xl shadow-apple dark:shadow-apple-dark overflow-hidden flex flex-col">
      <div className="relative h-48">
        <img
          src={
            article.image ||
            "https://placehold.co/500x300/e2e8f0/334155?text=No+Image"
          }
          alt={article.title}
          ...
          onError={(e) => {
            ...
            target.onerror = null;
            target.src =
              "https://placehold.co/500x300/e2e8f0/334155?text=Image+Error";
          }}
        />
      ...
      <div className="px-4 pb-4 flex justify-between items-center text-sm text-neutral-500 dark:text-neutral-400">
        <span>
          {article.source.name} • {formattedTime}
        </span>
